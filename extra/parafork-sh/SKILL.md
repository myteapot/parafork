---
name: parafork-sh
description: "维护者级、脚本优先的 Git worktree 工作流（bash/sh 版本），用于让 agent 在仓库中安全贡献：init/status/check/commit/pull/merge/debug。包含严格的 worktree-root guard、本地 merge 批准、remote base 对齐，以及防止提交 paradoc/.worktree-symbol。可在任意仓库直接运行（skill 本体不进仓库，仅创建 .parafork 会话容器目录）。"
---
