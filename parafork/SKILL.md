...
1) 在 base repo（主目录的任意子目录也可以）运行 `bash "<PARAFORK_SCRIPTS>/init.sh"`。  
2) 按 init 输出 `cd "<WORKTREE_ROOT>"` 进入 worktree 根目录。  
3) 运行 `bash "<PARAFORK_SCRIPTS>/status.sh"` 和 `bash "<PARAFORK_SCRIPTS>/check.sh" --phase plan`。  
4) 每个 task 按“微循环”推进：
   - 更新 `paradoc/Plan.md`
...
- `init.sh`：创建 worktree、写 `.worktree-symbol`、初始化 `paradoc/*`、写 ignore/exclude，并打印下一步。
...
- Plan 写作指南：`references/How-to-write-plan.md`
